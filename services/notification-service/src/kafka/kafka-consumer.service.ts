  ...
  onModuleInit() {
    this.consumer
      ...
      .then(async () => {
        ...
        this.logger.log(
          '🚀 Conectado exitosamente a Apache Kafka como Consumidor',
        );
        ...
        await this.consumer.run({
          eachMessage: async ({ topic, partition, message }) => {
            ...
            this.logger.log(
              `📥 Mensaje recibido en topic '${topic}' partition [${partition}]`,
            );
            if (value) {
              try {
                const payload = JSON.parse(value) as ReservationEmailPayload;
                this.logger.log(
                  `Procesando confirmación de correo para: ${payload.email}`,
                );
                await this.mailService.sendConfirmationEmail(payload);
              } catch (parseError) {
                this.logger.error(
                  `Error parsing Kafka message: ${(parseError as Error).message}`,
                );
              }
            }
          },
      ...
      .catch((error) => {
        this.logger.error(
          `❌ Falló la conexión/suscripción a Kafka: ${(error as Error).message}`,
        );
      });
  }
