...
export class KafkaConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.logger.log(`Inicializando consumidor Kafka con brokers: ${brokersEnv}`);
    this.kafka = new Kafka({
      clientId: 'notification-service',
      brokers: brokersEnv.split(','),
    });
    this.consumer = this.kafka.consumer({ groupId: 'notification-email-group' });
  }

  async onModuleInit() {
    try {
      await this.consumer.connect();
      await this.consumer.subscribe({ topic: 'reservation-emails', fromBeginning: false });
